...
@WebServlet(name = "FormularioRegistroDosPasos", urlPatterns = {"/FormularioRegistroDosPasos"})
public class FormularioRegistroDosPasos extends HttpServlet {
    ...
    "           <link href=\"https://fonts.googleapis.com/css?family=Lato|Roboto\" rel=\"stylesheet\">");              
            ...
            out.println("</showthem>");
            }else if(conteoErrores(errores)>0 && mapa.get("iteration") != null){
                for(String error : errores){
                    if(error.length()>0){
                        out.println("<span>"+error.replace("_","")+"</span>");
                        out.println("<br>");
                    }
                }
                out.println("<form action=\""+request.getContextPath()+"/FormularioRegistroDosPasos\" method=\"post\">");
                if(mapa.get("nombre_ob")[0].length()>0){
                    out.println("<input type=\"hidden\" id=\"nombre\" name=\"nombre_ob\" value=\""+mapa.get("nombre_ob")[0]+"\">");
                }else{
                    out.println("<input type=\"hidden\" id=\"nombre\" name=\"nombre_ob\" value=\"\">");
                }
                if(mapa.get("apellidos")[0].length()>0){
                    out.println("<input type=\"hidden\" id=\"apellidos\" name=\"apellidos\" value=\""+mapa.get("apellidos")[0]+"\">");
                }else{
                    out.println("<input type=\"hidden\" id=\"apellidos\" name=\"apellidos\" value=\"\">");
                }
                if(mapa.get("genero")[0].length()>0 && mapa.get("genero")[0].equals("Hombre")){
                    out.println("<input type=\"hidden\" id=\"gen_hombre\" name=\"genero\" value=\"Hombre\" checked>");
                }
                if(mapa.get("genero")[0].length()>0 && mapa.get("genero")[0].equals("Mujer")){
                    out.println("<input type=\"hidden\"  id=\"gen_mujer\" name=\"genero\" value=\"Mujer\" checked>");
                }
                if(errores[1].length()==0){
                    out.println("<input type=\"hidden\" id=\"checkDias\" name=\"Dia\" value=\""+mapa.get("Dia")[0]+"\"></select>");
                    out.println("<input type=\"hidden\" id=\"checkMeses\" name=\"Mes\" value=\""+mapa.get("Mes")[0]+"\"></select>");
                    out.println("<input type=\"hidden\" id=\"checkAnios\" name=\"Anio\" value=\""+mapa.get("Anio")[0]+"\"></select>");
                }else{
                    out.println("<input type=\"hidden\" id=\"checkDias\" name=\"Dia\"></select>");
                    out.println("<input type=\"hidden\" id=\"checkMeses\" name=\"Mes\"></select>");
                    out.println("<input type=\"hidden\" id=\"checkAnios\" name=\"Anio\"></select>");
                }
                if(mapa.get("usuario_ob")[0].length()>0){
                    out.println("<input type=\"hidden\" id=\"usuario_ob\" name=\"usuario_ob\" value=\""+mapa.get("usuario_ob")[0]+"\">");
                }else{
                    out.println("<input type=\"hidden\" id=\"usuario_ob\" name=\"usuario_ob\" value=\"\">");
                }
                if(mapa.get("password_ob")[0].length()>0){
                    out.println("<input type=\"hidden\" id=\"password_ob\" name=\"password_ob\" value=\""+mapa.get("password_ob")[0]+"\">");
                }else{
                    out.println("<input type=\"hidden\" id=\"password_ob\" name=\"password_ob\" value=\"\">");
                }
                if(chunkAficiones.contains("Deporte")){
                    out.println("<input type=\"hidden\" name=\"aficiones\" value=\"Deporte\" id=\"deporte\" checked>");
                }
                if(chunkAficiones.contains("Musica")){
                    out.println("<input type=\"hidden\" name=\"aficiones\" value=\"Musica\" id=\"musica\" checked>");
                }
                if(chunkAficiones.contains("Coches")){
                    out.println("<input type=\"hidden\" name=\"aficiones\" value=\"Coches\" id=\"coches\" checked>");
                }
                if(chunkAficiones.contains("Cine")){
                    out.println("<input type=\"hidden\" name=\"aficiones\" value=\"Cine\" id=\"cine\" checked>");
                }
...
"                        </script>");
                ...
                out.println("<form action=\""+request.getContextPath()+"/FormularioRegistroDosPasos\" method=\"post\">");
                    ...
                    out.println("<input type=\"hidden\" name=\"iteration\" value=\"comeAgain\">");
    ...
    public String[] errorChecker(String[] mapa){
        ...
        if(mapa[2].equals("2") && Integer.parseInt(mapa[1])==29 && Integer.parseInt(mapa[3])%4 != 0){
            errores[1] += "Ese año no es bisiesto_";
        }
        if(mapa[2].equals("2") && Integer.parseInt(mapa[1])>29){
            errores[1] += "Fecha incorrecta para mes de Febrero_";
        }
        if((mapa[2].equals("1") || mapa[2].equals("4") || mapa[2].equals("5") || mapa[2].equals("7") || mapa[2].equals("8") || mapa[2].equals("10") || mapa[2].equals("12")) && Integer.parseInt(mapa[1])>30){
            errores[1] += "Ese mes no tiene 31 días";
        }
